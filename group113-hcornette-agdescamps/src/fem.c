    ...
    */
    ...
    void geoMeshImport() 
    {
        ...
        for (int i=0; i < n/2; i++) {
            ...
            // theDomain->name[0] = '\0'; // Initialize name as an empty string
    ...
    // === Élimination de Gauss de la bande supérieur ===
    ...
    double* femBandSystemSolve(femBandSystem *sys) {
        ...
        for (int k = 0; k < size; k++) { // k = Ligne pivot
            ...
            for (int i = k + 1; i < size && i < k + band; i++) {
                ...
                double factor = factor_aik / pivot;
    ...
    femGraph* femBuildAdjacencyGraph(femMesh* elements, femMesh* edges, int nNodes) {
        ...
        graph->nNodes = nNodes;
        graph->adjLists = calloc(nNodes, sizeof(femNodeAdj*)); 
        graph->degree = calloc(nNodes, sizeof(int));
    ...
    typedef struct { int index; int degree; } NeighborInfo; // à ajouter dans fem.h
    ...
    int* femComputeRcmPermutation(femGraph* graph, int startNode) {
        ...
        while (head < tail) {
            ...
            NeighborInfo neighbors[graph->degree[u]]; 
            ...
            while (adj != NULL) {
                ...
                if (!visited[v]) {
                    ...
                    neighbor_count++;
                    //visited[v] = 1; // Marquer ici ou après le tri ?
                }
                adj = adj->next;
            }
